                        ...
                        <div class="col-md-8">
                            <div class="card">
                                <div class="card-header" data-background-color="red">
                                    <h4 class="title">Cambia horas clase de Materia</h4>
                                    <p class="category">Profesor: @foreach($prof as $p) {{$p->nomDoc}} @endforeach</p>
                                </div>
                                <div class="card-content">
                                    <form action="{{route('guardahoras')}}" method="POST">
                                        {{csrf_field()}} <!-- Para indicar el uso de formulario -->
                                        <div class="form-group  is-empty">
                                            <!-- <input type="select" class="form-control" placeholder="Ingresa nombre"> -->
                                            @foreach($horas as $h)
                                            <input type="hidden" name="idam" value=" {{$h->idam}}">
                                            <input type="hidden" name="idd" value=" {{$h->idd}}">
                                            <div class="form-group label-floating">
                                                <label class="control-label">Horas</label>
                                                <input type="text" name="horas" class="form-control" value="{{$h->horas}}">
                                            </div>
                                            @endforeach
                                            <span class="material-input"></span>
                                        </div>
                                        <br><button type="submit" class="btn btn-primary pull-right" data-background-color="red">Cambiar</button>
                                    </form>
                                </div>
