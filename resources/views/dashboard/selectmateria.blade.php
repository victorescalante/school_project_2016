...
@section('titulo')
	Seleccion de Materia para grupo
@endsection
						...
						<div class="col-md-8">
                            <div class="card">
                                <div class="card-header" data-background-color="red">
                                    <h4 class="title">Lista de materias</h4>
                                    <p class="category">{{$semestre}}° Semestre</p>
                                </div>
                                <div class="card-content">
                                    <form action="{{route('selectprofmat')}}" method="POST">
                                        {{csrf_field()}} <!-- Para indicar el uso de formulario -->
                                        <div class="form-group  is-empty">
                                            <input type="hidden" name="idgru" value="{{$idg}}"> 
                                            <input type="hidden" name="semestre" value="{{$semestre}}"> 
                                            <select class="form-control" name='idm'> -->
                                                @foreach($materias as $m)
                                                    <option value='{{$m->idm}}'>{{$m->nommat}}</option>
                                                @endforeach
                                            </select>
                                            <span class="material-input"></span>
                                        </div>
                                        <br><button type="submit" class="btn btn-primary pull-right" data-background-color="red">Seleccionar</button>
                                    </form>
                                </div>
